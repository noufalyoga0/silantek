...
<div class="row g-3">
    ...
    <div class="col-lg-4">
                ...
                <div class="mb-2">
                    @php
                        $roleLabel = match($user->role) {
                            ...
                            'pic_opd'      => ['PIC IT OPD',     'success'],
                            default        => [ucfirst($user->role), 'secondary'],
                        };
                    @endphp
                    <span class="badge bg-{{ $roleLabel[1] }}">{{ $roleLabel[0] }}</span>
                </div>
                <p class="text-muted small mb-0">{{ $user->opd->nama_opd ?? 'Tanpa OPD' }}</p>
            </div>
            ...
            <div class="card-body p-0">
                <div class="row g-0 text-center">
                    ...
                    <div class="col-6 p-3 border-bottom">
                        <div class="fw-bold fs-4 text-warning">{{ $stats['open'] }}</div>
                        <div class="text-muted" style="font-size:0.72rem;">Aktif</div>
                    </div>
                    <div class="col-6 p-3 border-end">
                        <div class="fw-bold fs-4 text-success">{{ $stats['resolved'] }}</div>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-0">
                @foreach($aktivitasTerakhir as $notif)
                <div class="d-flex align-items-start gap-2 p-3 border-bottom">
                    <div class="flex-shrink-0 mt-1">
                        ...
                        @endphp
                        <i class="bi {{ $ic }}" style="font-size:0.85rem;"></i>
                    </div>
                    <div class="flex-grow-1" style="overflow:hidden;">
                        <p class="mb-0 small text-truncate" style="max-width:100%;">{{ $notif->pesan }}</p>
                        <span class="text-muted" style="font-size:0.68rem;">{{ $notif->created_at->diffForHumans() }}</span>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="col-lg-8">
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-control bg-light">{{ $user->opd->nama_opd ?? '-' }}</div>
                    </div>
                </div>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('profil.ganti-password') }}" method="POST">
                    @csrf
                    <div class="row g-3">
