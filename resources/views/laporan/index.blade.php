...
@section('page-title', 'Laporan Insiden Bulanan')
        ...
        <form method="GET" action="{{ route('laporan.index') }}" class="row g-2 align-items-end">
                ...
                <select name="bulan" class="form-select form-select-sm">
                    @foreach(range(1,12) as $b)
                        <option value="{{ $b }}" {{ $bulan == $b ? 'selected' : '' }}>
                            {{ \Carbon\Carbon::createFromDate(2024, (int)$b, 1)->locale('id')->isoFormat('MMMM') }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-3">
                ...
                <select name="opd_id" class="form-select form-select-sm">
                    ...
                    @foreach($opd as $o)
                        <option value="{{ $o->id }}" {{ request('opd_id') == $o->id ? 'selected' : '' }}>
                            {{ $o->nama_opd }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100 text-center"
            ...
            <div class="card-body py-3">
                ...
                <div class="fw-bold" style="font-size:1.85rem; color:#2563eb; line-height:1;">{{ $stats['total'] }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100 text-center"
            ...
            <div class="card-body py-3">
                ...
                <div class="fw-bold" style="font-size:1.85rem; color:#d97706; line-height:1;">{{ $stats['open'] }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100 text-center"
            ...
            <div class="card-body py-3">
                ...
                <div class="fw-bold" style="font-size:1.85rem; color:#16a34a; line-height:1;">{{ $stats['resolved'] }}</div>
            ...
            <div class="card-body py-3">
                <div style="width:40px; height:40px;
                    ...
                    <i class="bi bi-exclamation-triangle-fill {{ $stats['overdue'] > 0 ? 'overdue-badge' : '' }}"
                       style="color:{{ $stats['overdue'] > 0 ? '#dc2626' : '#94a3b8' }}; font-size:1rem;"></i>
                </div>
                <div class="fw-bold" style="font-size:1.85rem; color:{{ $stats['overdue'] > 0 ? '#dc2626' : '#0f172a' }}; line-height:1;">{{ $stats['overdue'] }}</div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h6 class="mb-0 fw-semibold">
            ...
            Data Insiden —
            {{ \Carbon\Carbon::createFromDate((int)$tahun, (int)$bulan, 1)->locale('id')->isoFormat('MMMM') }} {{ $tahun }}
            ({{ $tiket->count() }} insiden)
        </h6>
    </div>
            ...
            <table class="table table-hover align-middle mb-0 small">
                ...
                <tbody>
                    ...
                    <tr class="{{ $t->is_overdue ? 'overdue-row' : '' }}">
                        ...
                        <td>
                            <a href="{{ route('tiket.show', $t) }}" class="text-decoration-none fw-semibold">
                                {{ $t->nomor_tiket }}
                            </a>
                        </td>
                        ...
                        <td>{{ $t->resolved_at ? $t->resolved_at->format('d/m/Y H:i:s') : '-' }}</td>
                        <td>
                            @if($t->resolved_at)
                                @php
